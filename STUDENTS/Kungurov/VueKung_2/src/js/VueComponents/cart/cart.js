...
export default {
    ...
    methods: {
		...
		async addItem(item) {
  			...
  			try {
  			  if (!find) {
  			    ...
  			    if(imgUrl){
                const data = await this.$api.send(this.urlCart, 'POST', newItem);
  			    if (!data.error) {
  			      this.itemsCart.push(newItem);
  			    }
  			  }
  			  } else {
                const data = await this.$api.send(this.urlCart, 'PUT', { value: 1 }, id);
  			    if (!data.error) {
  			      find.amount++;
  			    }
  			}
		...
		async deleteItem(item) {
           ...
           console.log(id)
		      try {
                const data = await this.$api.send(this.urlCart + `/${del}`, 'DELETE', {id});
				const find = this.itemsCart.find(cartItem => cartItem.id === id);
...
async created() {
    ...
    template: `
    <div>
    <div>Cart <span id="cart-counter">({{ value }})</span></div>
    ...
    <div id="cart" v-if="click">
      ...
      <cartItems
          ...
          />
      <div id ='mon'><span>{{money}}</span></div>
      </div>
      </div>
    `,
  };