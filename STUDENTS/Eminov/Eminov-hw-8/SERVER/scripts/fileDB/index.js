...
async function fileDB() {
    try {
        const data = await Promise.all([await readJSON(catalogURL), await readJSON(cartURL), await readJSON(menuURL), await readJSON(descriptionURL), await readJSON(contactURL)]);

        const [catalog, cart, menu, description, contact] = data;

        await Promise.all([await writeJSON(dbURL + '/catalog.json', catalog), await writeJSON(dbURL + '/cart.json', cart), await writeJSON(dbURL + '/menu.json', menu), await writeJSON(dbURL + '/description.json', description), await writeJSON(dbURL + '/contact.json', contact)]);
